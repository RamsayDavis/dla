...
###############################################################################
# Hierarchical bit-array helpers
###############################################################################


def _init_hierarchy(lmax: int) -> TypedList:
    """
    Initialize hierarchical bit-packed arrays.
    Each level uses 1 bit per site (8x memory reduction vs uint8).
    For a size x size grid, we need (size * size + 7) // 8 bytes.
    """
    ss = TypedList()
    for l in range(lmax):
        size = 1 << l
        ...
        ss.append(np.zeros(num_bytes, dtype=np.uint8))
    return ss


@njit(cache=True)
def _getslxy(ss: Sequence[np.ndarray], l: int, x: int, y: int) -> int:
    """
    Get bit at position (x, y) from bit-packed array.
    ...
    byte_val = ss[l][byte_index]
    # Extract bit (MSB is bit 0, so shift by 7 - bit_index)
    return (byte_val >> (7 - bit_index)) & 1


@njit(cache=True)
def _setslxy(ss: Sequence[np.ndarray], l: int, x: int, y: int) -> None:
    """
    Set bit at position (x, y) in bit-packed array.
    For level l, grid size is 2^l.
    """
    size = 1 << l
    if x < 0 or x >= size or y < 0 or y >= size:
    ...
    ss[l][byte_index] |= mask


@njit(cache=True)
def _mark(ss: Sequence[np.ndarray], lmax: int, x: int, y: int) -> None:
    for level in range(lmax - 1, -1, -1):
        _setslxy(ss, level, x, y)
        x >>= 1
        y >>= 1


@njit(cache=True)
def _mark_with_pattern(
    ss: Sequence[np.ndarray], lmax: int, x: int, y: int, pattern: np.ndarray
) -> None:
    _mark(ss, lmax, x, y)
    for i in range(pattern.shape[0]):
        _mark(ss, lmax, x + int(pattern[i, 0]), y + int(pattern[i, 1]))
...
def run_simulation_kernel(
    ...
    _mark_with_pattern(ss, lmax, x_cen, y_cen, agg_pattern)
        ...
        while True:
            ...
            if is_inside:
                if diff_stepsize == FIXED_STEPSIZE:
                    x_cur, y_cur = _diffuse_fixed(
                        ss, lmax, x_cur, y_cur, alias_fn, alias_an
                    )
                else:
                    x_cur, y_cur = _diffuse_variable(
                        ss, lmax, x_cur, y_cur, alias_fn, alias_an
                    )
            else:
            ...
            if (
                ...
                # Inline _getslxy logic
                linear_idx = x_cur * (1 << (lmax - 1)) + y_cur
                byte_val = ss[lmax - 1][linear_idx // 8]
                is_sticky = (byte_val >> (7 - (linear_idx % 8))) & 1

                if is_sticky:
                    # HIT! Freeze it.
                    _mark_with_pattern(ss, lmax, x_cur, y_cur, agg_pattern)
...
class KohLatticeSimulator:
    """
    Python/Numba port of koh_lattice.cc.
    ...
    def __init__(
        ...
        *,
        ...
        self.ss = _init_hierarchy(self.lmax)
